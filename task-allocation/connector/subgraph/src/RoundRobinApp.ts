...
import { populateTaskDataFromContract, getTaskEntity, getUserEntity, getUserRejectedTaskEntity } from './helpers'
...
export function handleTaskDeleted(event: TaskDeletedEvent): void {
    ...
    log.debug(
        'TaskDeleted event received. taskId: {}',
        [event.params.taskId.toString()]
    )
...
export function handleTaskAccepted(event: TaskAcceptedEvent): void {
    ...
    log.debug(
        'TaskAccepted event received. userId: {} taskId: {}',
        [event.params.userId.toString(), event.params.taskId.toString()]
    )
...
export function handleTaskRejected(event: TaskRejectedEvent): void {
    const userRejectedTask = getUserRejectedTaskEntity(event.address, event.params.userId, event.params.taskId)
    const task = getTaskEntity(event.address, event.params.taskId)

    log.debug(
        'TaskRejected event received. userId: {} taskId: {}',
        [event.params.userId.toString(), event.params.taskId.toString()]
    )
...
export function handleTaskAllocated(event: TaskAllocatedEvent): void {
    ...
    log.debug(
        'TaskAllocated event received. userId: {} taskId: {}',
        [event.params.userId.toString(), event.params.taskId.toString()]
    )
...
export function handleUserRegistered(event: UserRegisteredEvent): void {
    ...
    log.debug(
        'UserRegistered event received. userId: {}',
...
export function handleUserDeleted(event: UserDeletedEvent): void {
    ...
    log.debug(
        'UserDeleted event received. userId: {}',
        [event.params.userId.toString()]
    )
