import { hexToString, isHex } from 'web3-utils'
...
export const hexPropertiesToString = obj => {
  return Object.keys(obj).reduce((transformedState, currKey) => {
    const value = obj[currKey]
    if (isHex(value)) {
      transformedState[currKey] = hexToString(value)
    } else {
      transformedState[currKey] = value
    }
    return transformedState
  }, {})
}

export const removeNumberProperties = obj => {
  const newObj = {}
  for (const key in obj) {
    const res = parseInt(key)
    if (isNaN(res)) {
      newObj[key] = obj[key]
    }
  }
